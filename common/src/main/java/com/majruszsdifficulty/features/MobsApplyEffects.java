...
public class MobsApplyEffects {
	private static List< ExtraEffectDef > EXTRA_EFFECTS = List.of(
		new ExtraEffectDef(
			...
			true,
			EntityType.SPIDER,
			new EffectDef( ()->MobEffects.POISON.value(), 0, 7.0f )
		),
		new ExtraEffectDef(
			GameStage.EXPERT_ID,
			0.5f,
			true,
			EntityType.SLIME,
			new EffectDef( ()->MobEffects.MOVEMENT_SLOWDOWN.value(), 0, 7.0f )
		),
		new ExtraEffectDef(
			GameStage.MASTER_ID,
			0.5f,
			true,
			EntityType.SHULKER,
			new EffectDef( ()->MobEffects.BLINDNESS.value(), 0, 7.0f )
		),
		new ExtraEffectDef(
			GameStage.MASTER_ID,
			0.75f,
			true,
			EntityType.PHANTOM,
			new EffectDef( ()->MobEffects.LEVITATION.value(), 0, 7.0f )
		)
	);
