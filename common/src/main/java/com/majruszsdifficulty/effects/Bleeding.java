...
public class Bleeding extends MobEffect {
	...
	public static boolean canApplyTo( LivingEntity entity ) {
		return BleedingConfig.IS_APPLICABLE_TO_ANIMALS && entity instanceof Animal && !BleedingConfig.IMMUNE_MOBS.contains( entity.getType() )
			|| BleedingConfig.IS_APPLICABLE_TO_ILLAGERS && entity.getType().is( EntityTypeTags.ILLAGER ) && !BleedingConfig.IMMUNE_MOBS.contains( entity.getType() )
	...
	public static class MobEffectInstance extends net.minecraft.world.effect.MobEffectInstance {
		...
		public MobEffectInstance( int duration, int amplifier, @Nullable LivingEntity attacker ) {
			super( BuiltInRegistries.MOB_EFFECT.wrapAsHolder( MajruszsDifficulty.BLEEDING_EFFECT.get() ), duration, amplifier, false, false, true );
